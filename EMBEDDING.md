...
└── takoyaki-cms/           ← ★CMSをここに配置
    ├── config.php          ← 環境ごとの設定（ここだけ編集）
    ├── functions.php       ← 共通ヘルパー（config.php から自動で読まれる）
    ├── admin/              ← 編集者はここにアクセス
    └── uploads/            ← 画像保存先
```
...
```
takoyaki-cms/
├── config.php              # 環境ごとの設定値のみ
├── functions.php           # DB接続 + 共通ヘルパー
...
| 項目 | 状態 |
|------|------|
| **TESTING.md の v2.0.0 対応** | ✅ **完了**（下記「2026-07-28 (3)」参照） |
...
### 2026-07-28 (3) TESTING.md の作り直し

v1.x 向けだった手順書を v2.0.0 の実装に合わせて全面的に書き直した。

**構成**: 17章 → 16章 + 付録3つ。

| 削除した章 | 追加した章 |
|-----------|-----------|
| パスワードリセット / ゴミ箱 / ユーザー管理・ロール / 監査ログ / バックアップ・プラグイン / きれいなURL / RSS・sitemap | **作品メタと動画埋め込み**（3章）/ **本文セクション**（4章）/ **スキル管理**（10章）/ **v1.x からの移行確認**（16章）/ **付録C 回帰テスト** |

工夫した点:

- **付録C に回帰テスト（R-1〜R-15）を新設**。v2.0.0 で直した不具合ごとに
  「どう確認すれば再発が分かるか」を1行にまとめた。全16章を回す時間が無いときは
  ここだけ通せばよい
- 各章に「**これは v1.x で壊れていた箇所です（B-1）**」という注記を入れ、
  手順書と評価ドキュメントの対応が追えるようにした
- 冒頭に所要時間つきの目次と、「時間が無いときは 0 → 2 → 4 → 10 → 14 → 付録C」の
  短縮ルートを用意した
- MAMP の MySQL クライアントの実際のパス
  （`/Applications/MAMP/Library/bin/mysql80/bin/mysql`）と、TCP・ソケット両方の
  接続方法を記載した（旧版は TCP のみ記載で、パスの記載が無かった）

#### 手順書の記述を実際に叩いて検証した

「書いてある期待結果が実装と一致するか」を curl で確認した。
バリデーションメッセージ4種、slug の採番3パターン、設定値の補正はすべて記載どおりだった。

#### 検証中に見つけた不具合（修正済み）

**数字だけの slug が作られ、ID検索と衝突していた。**

カテゴリ名 `ブログ2` を追加すると、`sluggify()` が数字だけを残して slug が `2` になった。
`get_category()` / `get_post()` は `is_numeric()` で「IDか slug か」を判定するため、
**slug `2` のカテゴリを指定すると id=2 の別カテゴリが引かれる**。

B-3（カテゴリ slug の数字化）を直したつもりが、数字を含む名前で同じ状態が再現していた。

`unique_slug()` に1行足して修正:
...
`unique_slug()` は作品・カテゴリ・タグで共通なので、3つとも同時に直った。
確認結果:

| 入力 | 修正前の slug | 修正後の slug |
|------|-------------|-------------|
| カテゴリ `ブログ2` | `2` ❌ | `category` |
| カテゴリ `2026` | `2026` ❌ | `category-3` |
| 作品 `2026` | `2026` ❌ | `work-47` |
| タグ `2026` | `2026` ❌ | `tag` |

TESTING.md にもこのケースを 8-5 として追加した。

### 次にやると効果が大きいこと

1. **E-4 の自動テスト化** — 今回も検証を使い捨てスクリプトで行った。
   `tests/run.php` として残せば、次の変更から再利用できる。
   今回の `unique_slug()` の不具合は、まさに自動テストがあれば早く気づけた種類のもの
2. **EMBEDDING.md 本文の書き直し** — 既存サイトへの組み込みを実際にやる段階で
